...
# Remove any previously added title or legend groups if they exist
for el in list(root):
    if el.attrib.get('id') in ['map_legend', 'map_title']:
        root.remove(el)
...
# 1. Harmonic modern departmental pastel palette
dept_colors = {
...
# Apply department fills and dots
for el in root.iter():
...
# 2. Cartographic Legend in top-left empty space (costado izquierdo superior)
...
# 3. Title Card in top-right empty space (costado superior derecho)
...
print(f"Successfully overwritten {base_svg}!")

# Render preview PNG to pruebas/
doc = pymupdf.open(base_svg)
page = doc[0]
pix = page.get_pixmap(dpi=150)
pix.save('pruebas/rionegro_final_preview.png')
print('Generated pruebas/rionegro_final_preview.png successfully:', pix.width, 'x', pix.height)
