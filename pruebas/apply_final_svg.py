...
for el in list(root):
    if el.attrib.get('id') == 'map_legend':
        root.remove(el)

# Place legend in upper left corner (costado izquierdo superior)
...
# Save directly to target file
tree.write(svg_target, encoding='utf-8', xml_declaration=True)
print(f"Updated {svg_target} successfully!")
