...
#show heading.where(level: 1): it => [
  ...
  #h(5pt)
  #text(weight: "bold", size: 11.2pt, fill: primary)[#it.body]
  #v(4pt)
]

#show heading.where(level: 2): it => [
  #v(7pt)
  #text(weight: "bold", size: 9.6pt, fill: primary)[#it.body]
  #v(3pt)
]
...
#page(
  ...
  #block(
    ...
    [
      ...
      #v(4pt)
      #text(size: 17.5pt, weight: "bold", fill: primary, hyphenate: false)[
        Avance 2: Modelo de Gestión, Gobernanza y Poligobernanza
      ] \
      #v(4pt)
      #text(size: 10.8pt, fill: rgb("#285082"), weight: "regular")[
...
#set page(
  paper: "a4",
  margin: (x: 2.2cm, top: 2.2cm, bottom: 2.3cm),
  header: [
    #grid(
      columns: (1fr, auto),
      align: (left + horizon, right + horizon),
...
#table(
  columns: (1.15fr, 1.35fr, 1.85fr, 1.8fr),
  fill: (col, row) => if row == 0 { primary } else if calc.even(row) { bg-card } else { white },
  stroke: 0.5pt + border-subtle,
  inset: (x: 6.5pt, y: 5pt),
  [#text(fill: white, weight: "bold", size: 8pt)[Dimensión]],
  [#text(fill: white, weight: "bold", size: 8pt)[Paradigma Teórico]],
  [#text(fill: white, weight: "bold", size: 8pt)[Manifestación Real en CURZAS]],
  [#text(fill: white, weight: "bold", size: 8pt)[Tensión o Desafío Central]],
...
1. *Una sólida columna vertebral burocrático-weberiana y de autogobierno reformista:* Garantiza la estabilidad, la legalidad formal, la vigencia de los derechos colectivos de trabajo y la legitimidad democrática interna a través del cogobierno cuatripartito.
2. *Una modernización instrumental electrónica en curso:* Ha optimizado la gestión académica y el expediente digital (SIU, SUDOCU), pero aún presenta el desafío crítico de evolucionar desde las actuales "islas tecnológicas" hacia una verdadera arquitectura de *Estado Plataforma e interoperabilidad de datos abiertos* con el sistema científico y los gobiernos locales.
3. *Emergentes dinámicas de poligobernanza y redes ad hoc:* A través de las Secretarías de Extensión e Investigación, el CURZAS demuestra capacidad para sortear la rigidez de los departamentos tradicionales, desplegando tramados transdisciplinarios y temporales con los municipios, cooperativas y comunidades de la Patagonia Norte y la Línea Sur, configurando el embrión de una gobernanza pública abierta y territorialmente situada.
...
#rect(
  ...
  #text(weight: "bold", size: 11pt, fill: primary)[Bibliografía y Fuentes Consultadas]
  #v(5pt)
  #line(length: 100%, stroke: 0.6pt + accent)
  #v(8pt)
  
  #set text(size: 8.5pt, fill: text-main)
