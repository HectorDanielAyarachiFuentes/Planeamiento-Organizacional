...
# Set clean viewBox covering all elements including the western tip of Bariloche
# Bbox of map is x: -70 to 1750, y: 20 to 1200
...
for el in root.iter():
    ...
    if tid == 'text2212':  # San Antonio (Department name)
        # Move up so it doesn't collide with the legend
        el.attrib['x'] = "1095"
        el.attrib['y'] = "875"
        for ts in el.iter():
            if ts.tag.endswith('tspan'):
...
test_svg_path = 'pruebas/RioNegro_improved_v3.svg'
tree.write(test_svg_path, encoding='utf-8', xml_declaration=True)

doc = pymupdf.open(test_svg_path)
page = doc[0]
pix = page.get_pixmap(dpi=150)
pix.save('pruebas/rionegro_improved_v3.png')
print('Generated rionegro_improved_v3.png successfully')
